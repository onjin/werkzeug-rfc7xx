...
ERRORS = [
    ...
    (704, "Goto Fail"),
    (705, (
        "I wrote the code and missed the necessary validation by an "
        "oversight (see 795)"
    )),
...
RE_SKIP_CHARS = re.compile("[/!?,:,;#\(\)_-]")


def _msg_to_class_name(msg):
    msg = RE_SKIP_CHARS.sub(" ", msg)
    msg = msg.replace("'", "")
    msg = msg.replace('"', "")
    msg = msg.replace('.', "")
    return "".join([w.capitalize() for w in msg.rsplit()])


for code, msg in ERRORS:
    class_name = _msg_to_class_name(msg)
    class_obj = type(class_name, (HTTPException, ), {
        'code': code,
        'description': msg,
    })
